...
if __name__ == "__main__":
    if fly:
        ...
        tello.move_up(20)
    while True: 
        dist = tello.get_current_state()['tof']
        frame_read = tello.get_frame_read()
        myFrame = frame_read.frame
        img = cv2.resize(myFrame, (width, height))
        left_right_vel, forward_vel, up_down_vel, yaw_velocity = fly_drone(img, dist)
        tello.send_rc_control(left_right_vel, forward_vel, up_down_vel, yaw_velocity)